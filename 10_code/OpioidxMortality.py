...
# %%
pre_fl = plotting_chart(2010, 'blue', death_FL_pre, 'Death_per_cap', 'Year', legend= 'Florida',alpha=0.05)
post_fl = plotting_chart(2010, 'blue', death_FL_post, 'Death_per_cap', 'Year', legend ='Florida',  alpha=0.05)

final = (pre_fl + post_fl)

final.properties(title = "Pre-Post Analysis of Opioid Regulations on Mortality for Florida")
...
# %%
pre_wa = plotting_chart(2011, 'blue', death_WA_pre, 'Death_per_cap','Year', "Washington",alpha=0.05)
post_wa = plotting_chart(2011, 'blue', death_WA_post, 'Death_per_cap', 'Year', "Washington", alpha=0.05)

final = (pre_wa + post_wa)

final.properties(title = "Pre-Post Analysis of Opioid Regulations on Mortality for Washington")
...
# %%
pre_TX = plotting_chart(2007, 'orange', death_TX_pre, 'Death_per_cap', 'Year', "Texas", alpha=0.05)
post_TX = plotting_chart(2007, 'orange', death_TX_post, 'Death_per_cap', 'Year', "Texas",alpha=0.05)
...
# %%
pre_FL = plotting_chart(2010, 'blue', diff_FL_treat_pre, 'Death_per_cap', 'Year', "Florida",alpha=0.05)
post_FL = plotting_chart(2010, 'blue', diff_FL_treat_post, 'Death_per_cap', 'Year',"Florida", alpha=0.05)
pre_control = plotting_chart(2010, '#9467bd', diff_FL_control_pre, 'Death_per_cap', 'Year', "Comparison States - MI, NV, SC",alpha=0.05)
post_control = plotting_chart(2010, '#9467bd', diff_FL_control_post, 'Death_per_cap', 'Year', "Comparison States - MI, NV, SC", alpha=0.05)


final = pre_FL + post_FL + pre_control + post_control
final.properties(title = "Diff in Diff Analysis of Opioid Regulations on Mortality for Florida vs Comparison States")


# %%
diff_TX_data = death_data.loc[death_data['State_Code'].isin(["CA","NY","OR","TX"])].copy()
diff_TX_data['Treat'] = 1
diff_TX_data.loc[diff_TX_data['State_Code'].isin(["CA","NY","OR"]), 'Treat'] = 0
...
diff_TX_control_post = diff_TX_control.loc[diff_TX_control['Year'].isin(range(2007,2014))]

pre_TX = plotting_chart(2007, 'blue', diff_TX_treat_pre, 'Death_per_cap', 'Year', "Texas",alpha=0.05)
post_TX = plotting_chart(2007, 'blue', diff_TX_treat_post, 'Death_per_cap', 'Year',"Texas", alpha=0.05)
pre_control = plotting_chart(2007, '#9467bd', diff_TX_control_pre, 'Death_per_cap', 'Year', "Comparison States - CA, NY, OR",alpha=0.05)
post_control = plotting_chart(2007, '#9467bd', diff_TX_control_post, 'Death_per_cap', 'Year', "Comparison States - CA, NY, OR", alpha=0.05)


final = pre_TX + post_TX + pre_control + post_control
final.properties(title = "Diff in Diff Analysis of Opioid Regulations on Mortality for Texas vs Comparison States")


# %%
diff_WA_data = death_data.loc[death_data['State_Code'].isin(["HI","MT","OR","WA"])].copy()
diff_WA_data['Treat'] = 1
diff_WA_data.loc[diff_WA_data['State_Code'].isin(["HI","MT","OR"]), 'Treat'] = 0
...
pre_WA = plotting_chart(2011, 'blue', diff_WA_treat_pre, 'Death_per_cap', 'Year', "Washington",alpha=0.05)
post_WA = plotting_chart(2011, 'blue', diff_WA_treat_post, 'Death_per_cap', 'Year',"Washington", alpha=0.05)
pre_control = plotting_chart(2011, '#9467bd', diff_WA_control_pre, 'Death_per_cap', 'Year', "Comparison States - HI, MI, OR",alpha=0.05)
post_control = plotting_chart(2011, '#9467bd', diff_WA_control_post, 'Death_per_cap', 'Year', "Comparison States - HI, MI, OR", alpha=0.05)


final = pre_WA + post_WA + pre_control + post_control
final.properties(title = "Diff in Diff Analysis of Opioid Regulations on Mortality for Washington vs Comparison States")
